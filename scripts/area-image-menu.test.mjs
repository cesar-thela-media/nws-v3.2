...
const navbar = fs.readFileSync(path.join(root, "src/components/shadcn-space/blocks/navbar-07/index.tsx"), "utf8");

test("linked area records have unique explicit hero image associations", () => {
  const slugs = ["richmond-tx", "sugar-land-tx", "katy-tx", "fulshear-tx", "west-side-of-houston-tx", "cinco-ranch-tx", "rosenberg-tx", "weston-lakes-tx", "park-row-tx"];
...
test("areas desktop menu uses a two-column layout and retains canonical registry", () => {
  ...
  assert.match(navbar, /title: "Areas"/);
  assert.match(navbar, /hasDropdown: true/);
});
